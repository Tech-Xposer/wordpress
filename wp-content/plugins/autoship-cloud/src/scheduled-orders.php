...
function autoship_get_formatted_order_display_totals( $totals = array(), $currency = 'USD' ){
  ...
  foreach ( $defaults as $key => $values) {
    ...
    if ( isset( $totals[$key] ) ){
      $new_totals[$key]['value'] = autoship_get_formatted_price( $totals[$key], array( 'currency' => $currency ) );
...
function autoship_format_free_shipping_order_display_total( $totals ){
  ...
  if ( !array_key_exists( 'shipping_total', $totals ) && !empty( $totals ) ){
    ...
    $totals = autoship_array_insert_before( 'total', $totals, 'shipping_total', array('label'=>__('Shipping Total', 'autoship'), 'value'=>'Free' ) );
...
function autoship_get_calculated_scheduled_order_totals( $scheduled_order, $scheduled_items = array() ){
  ...
  $totals = array(
      ...
      'shipping_total' => $scheduled_order['shippingTotal'],
...
function autoship_get_schedulable_products_display_labels( $available_products, $products ){
  ...
  foreach ( $products as $product ){
    ...
    $available_products[$product['id']]['item_original_price'] = autoship_get_adjusted_product_item_price( $product['price'], $product['id'] );

    // Get the sale / current price
    $available_products[$product['id']]['item_price'] = $product['salePrice'] ?
    autoship_get_adjusted_product_item_price( $product['salePrice'], $product['id'] ) : $available_products[$product['id']]['item_original_price'];

    $available_products[$product['id']]['label'] = apply_filters( 'autoship_scheduled_order_form_item_add_display_name',
    $product['title'] . ' ' . autoship_get_formatted_price( $available_products[$product['id']]['item_price'], array(
...
if ( !function_exists( 'autoship_filter_schedulable_products_data' ) ){
   ...
   * @param array  An Array of stdClass objects.
   * @return array The filtered array
   */
...
function autoship_confirm_delete_schedule_notice_orders_html_wrapper ( $notice , $scheduled_order ){
  ...
  ob_start();?>
    ...
    <tr class="autoship-notice-row woocommerce-orders-table__row">
      ...
      <td class="autoship-order-notice action-notice" data-title="<?php echo __( 'Notice', 'autoship' ); ?>" colspan="<?php echo count( $columns ) - 1;?>"><p role="alert"><?php echo $notice; ?></p></td>
...
function autoship_confirm_delete_schedule_notice_order_html_wrapper ( $notice , $scheduled_order ){
  ...
  <div class="autoship-notice-block">
    ...
    <div class="autoship-order-notice action-notice" data-title="<?php echo __( 'Notice', 'autoship' ); ?>"><p role="alert"><?php echo $notice; ?></p></div>
