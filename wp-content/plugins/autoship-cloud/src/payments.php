...
function autoship_get_order_payment_data( $order_id ) {
  ...
  // Get the Gateway ID from the order.
	$payment_method_id = get_post_meta( $order_id, '_payment_method', true );
 ...
 */
function autoship_get_payment_method_cod_order_payment_data( $order_id ) {
 ...
 * @return QPilotPaymentData
 */
function autoship_get_braintree_paypal_order_payment_data( $order_id ) {
	$token_string       = get_post_meta( $order_id, '_wc_braintree_paypal_payment_token', true );
	$paypal_customer_id = get_post_meta( $order_id, '_wc_braintree_paypal_customer_id', true );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_braintree_order_payment_data( $order_id ) {
	$token_string = get_post_meta( $order_id, '_wc_braintree_credit_card_payment_token', true );
	$customer_id  = get_post_meta( $order_id, '_wc_braintree_credit_card_customer_id', true );
	if ( ! empty( $token_string ) && ! empty( $customer_id ) ) {
		$payment_data                      = new QPilotPaymentData();
		$card_type                         = get_post_meta( $order_id, '_wc_braintree_credit_card_card_type', true );
		$last_four                         = get_post_meta( $order_id, '_wc_braintree_credit_card_account_four', true );

    // Expiration in 2025-02 format.
		$expiry_date                       = get_post_meta( $order_id, '_wc_braintree_credit_card_card_expiry_date', true );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_ppec_order_payment_data( $order_id ) {
	$payment_agreement_id = get_post_meta( $order_id, '_ppec_billing_agreement_id', true );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_ppcp_order_payment_data( $order_id ) {

	$payment_token = get_post_meta( $order_id, 'payment_token_id', true );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_ppcp_cc_order_payment_data( $order_id ) {
 ...
 * @return QPilotPaymentData
 */
function autoship_get_authorize_net_order_payment_data( $order_id ) {

	$token_string = get_post_meta( $order_id, '_wc_authorize_net_cim_credit_card_payment_token', true );
	$customer_id  = get_post_meta( $order_id, '_wc_authorize_net_cim_credit_card_customer_id', true );

	if ( ! empty( $token_string ) && ! empty( $customer_id ) ) {

		$payment_data                      = new QPilotPaymentData();
		$card_type                         = get_post_meta( $order_id, '_wc_authorize_net_cim_credit_card_card_type', true );
		$last_four                         = get_post_meta( $order_id, '_wc_authorize_net_cim_credit_card_account_four', true );

    // Authnet Stores Expiration in YY-MM format so we need to adjust for Autoship
    // Expiration in format 23-02 should be 0223
		$expiry_date                       = get_post_meta( $order_id, '_wc_authorize_net_cim_credit_card_card_expiry_date', true );
    $expiration                        = explode( '-', $expiry_date );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_cybersource_cc_order_payment_data( $order_id ) {
	$token_string = get_post_meta( $order_id, '_wc_cybersource_credit_card_payment_token', true );
	$customer_id  = get_post_meta( $order_id, '_wc_cybersource_credit_card_customer_id', true );
	if ( ! empty( $token_string ) && ! empty( $customer_id ) ) {
		$payment_data                      = new QPilotPaymentData();
		$card_type                         = get_post_meta( $order_id, '_wc_cybersource_credit_card_card_type', true );
		$last_four                         = get_post_meta( $order_id, '_wc_cybersource_credit_card_account_four', true );

    // Cybersource Stores Expiration in YY-MM format so we need to adjust for Autoship
    // Expiration in format 23-02 should be 0223
		$expiry_date                       = get_post_meta( $order_id, '_wc_cybersource_credit_card_card_expiry_date', true );
    $expiration                        = explode( '-', $expiry_date );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_stripe_order_payment_data( $order_id ) {

	// Stripe version >= 4.0.0
	$token_id = get_post_meta( $order_id, '_stripe_source_id', true );

	// Stripe version < 4.0.0
	if ( empty( $token_id ) ) {
		$token_id = get_post_meta( $order_id, '_stripe_card_id', true );
	}
 ...
 * @return QPilotPaymentData
 */
function autoship_get_stripe_sepa_order_payment_data( $order_id ) {
 ...
 * @return QPilotPaymentData
 */
function autoship_get_checkout_com_order_payment_data( $order_id ) {

	// Retrieve the Payment ID ( i.e. transaction id )
	$transaction_id = get_post_meta( $order_id, '_cko_payment_id', true );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_test_gateway_order_payment_data( $order_id ) {
	$gateway_customer_id = get_post_meta( $order_id, '_autoship_test_gateway_customer_id', true );
	$gateway_payment_id  = get_post_meta( $order_id, '_autoship_test_gateway_payment_id', true );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_braintree_gateway_order_payment_data( $order_id ) {
	$payment_data = new QPilotPaymentData();

	$cc_token = get_post_meta( $order_id, '_wc_braintree_paypal_payment_token', true );
	$pp_token = get_post_meta( $order_id, '_wc_braintree_paypal_payment_token', true );

	// See if it's CC or PP
	if ( ! empty( $cc_token ) ) {
		$cc_card_type                      = get_post_meta( $order_id, '_wc_braintree_credit_card_card_type', true );
		$cc_last_four                      = get_post_meta( $order_id, '_wc_braintree_credit_card_account_four', true );
		$cc_expy                           = get_post_meta( $order_id, '_wc_braintree_credit_card_card_expiry_date', true );
		$cc_customer_id                    = get_post_meta( $order_id, '_wc_braintree_credit_card_customer_id', true );
		$payment_data->type                = 'Braintree';
		...
		$pp_expy                           = get_post_meta( $order_id, '_wc_braintree_credit_card_card_expiry_date', true );
		$pp_customer_id                    = get_post_meta( $order_id, '_wc_braintree_paypal_customer_id', true );
		$payment_data->type                = 'Braintree';
 ...
 * @return QPilotPaymentData
 */
function autoship_get_cybersource_order_payment_data( $order_id ) {
	$token_id = get_post_meta( $order_id, 'cybersource_token_id', true );
	if ( ! empty( $token_id ) ) {
 ...
 * @return QPilotPaymentData
 */
function autoship_get_nmi_order_payment_data( $order_id ) {

  global $wpdb;

  $token_id      = get_post_meta( $order_id, '_wc_nmi_gateway_woocommerce_credit_card_payment_token', true );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_sagepaymentsusaapi_order_payment_data( $order_id ) {

  $token = null;

  // Get the Sage / Paya Token & Customer ID
  $order_token_id   = get_post_meta( $order_id, '_SageToken', true );
  $customer_id      = get_post_meta( $order_id, '_customer_user', true );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_trustcommerce_order_payment_data( $order_id ) {

  // Grab the Token from the order.
 	$token_id  = get_post_meta( $order_id, '_trustcommerce_customer_id', true);
 ...
 * @return QPilotPaymentData
 */
function autoship_get_square_order_payment_data( $order_id ) {

  // Grab the Customer ID and Token from the order.
 	$customer_id  = get_post_meta( $order_id, '_wc_square_credit_card_customer_id', true);
 	$token_id     = get_post_meta( $order_id, '_wc_square_credit_card_payment_token', true);


	if ( ! empty( $token_id ) ) {
    $payment_data                      = new QPilotPaymentData();
    $card_type                         = get_post_meta( $order_id, '_wc_square_credit_card_card_type', true );
    $last_four                         = get_post_meta( $order_id, '_wc_square_credit_card_account_four', true );
    $expiry_date                       = get_post_meta( $order_id, '_wc_square_credit_card_card_expiry_date', true );
...
function autoship_sagepaydirect_order_payment_data_saved_payment_patch( $order_id, $posted_data, $order ){
  ...
  if( $sage_card_token !== 'new' && $sage_card_token ) {
    ...
    update_post_meta( $order->get_id(), '_SagePayDirectToken', $token->get_token() );
 ...
 * @return QPilotPaymentData
 */
function autoship_get_sage_order_payment_data( $order_id ) {

  // Grab the Customer ID and Token from the order.
 	$token_id = get_post_meta( $order_id, '_SagePayDirectToken', true);
...
function autoship_add_adjusted_gateway_metadata_ppec_paypal( $payment_meta, $order, $payment_method ){
  ...
  update_post_meta( $order->get_id(), '_woo_pp_txnData', $refund_data );
...
function autoship_add_adjusted_gateway_metadata_ppep_paypal( $payment_meta, $order, $payment_method ){
  ...
  update_post_meta( $order->get_id(), 'payment_token_id', $payment_method['gatewayPaymentId'] );
  update_post_meta( $order->get_id(), '_ppcp_paypal_order_id', $payment_meta['id'] );
  update_post_meta( $order->get_id(), '_ppcp_paypal_intent', 'CAPTURE' );
  ...
  update_post_meta( $order->get_id(), '_ppcp_paypal_payment_mode', 'sandbox' );
...
function autoship_add_adjusted_gateway_metadata_authnet( $payment_meta, $order, $payment_method ){
  ...
  $metadata = array(
    ...
    '_transaction_id'                                       => $payment_meta['transId']
  );
...
function autoship_add_adjusted_gateway_metadata_cybersource_cc( $payment_meta, $order, $payment_method ){
  ...
  $metadata = array(
    ...
    '_transaction_id'                                       => $transaction_data->id
  );
...
function autoship_add_adjusted_gateway_metadata_stripe( $payment_meta, $order, $payment_method ){
  ...
  if ( isset( $payment_meta['CustomerId'] ) ){
    $metadata = array(
      ...
      '_transaction_id'         => $payment_meta['Id']
    );
...
function autoship_add_adjusted_gateway_metadata_nmi_gateway_woocommerce_credit_card( $payment_meta, $order, $payment_method ){
  ...
  $metadata = array(
    ...
    '_payment_method'                                         => 'nmi_gateway_woocommerce_credit_card',
  );
...
function autoship_add_adjusted_gateway_metadata_braintree_credit_card( $payment_meta, $order, $payment_method ){
  ...
  $metadata = array(
    '_payment_method'                     => 'braintree_credit_card',
  ...
  if ( isset( $payment_meta['Target'] ) && isset( $payment_meta['Target']['Id'] ) ){
    ...
    $metadata['_transaction_id'] = $payment_meta['Target']['Id'];
  }
...
function autoship_add_adjusted_gateway_metadata_braintree_paypal( $payment_meta, $order, $payment_method ){
  ...
  $metadata = array(
    ...
    '_transaction_id'                     => $payment_meta['Target']['Id'],
    '_payment_method'                     => 'braintree_paypal',
...
function autoship_add_adjusted_gateway_metadata_sagepaymentsusaapi( $payment_meta, $order, $payment_method ){
  ...
  $metadata = array(
    '_payment_method'                     => 'sagepaymentsusaapi',
    '_payment_method_title'               => 'Credit Card via Paya'
  );
...
function autoship_add_adjusted_gateway_metadata_sage( $payment_meta, $order, $payment_method ){
  ...
  $metadata = array(
    '_payment_method'                     => 'sagepaydirect',
    '_payment_method_title'               => 'Credit Card via Sage',
...
function autoship_add_adjusted_gateway_metadata_checkout( $payment_meta, $order, $payment_method ){
  ...
  $metadata = array(
    '_payment_method'         => 'wc_checkout_com_cards',
    '_payment_method_title'   => 'Pay by Card with Checkout.com',
    '_cko_payment_id'         => $payment_meta['Id'],
    'cko_payment_authorized'  => true,
    'cko_payment_captured'    => true,
    '_transaction_id'         => $payment_meta['action_id'],
  );
